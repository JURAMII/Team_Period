...
import React, { useState } from "react";
import { Link, useParams } from "react-router-dom";
import { galleryImages } from "../../../image"; // 이미지 파일 import
import "./gallery.css";
import { Swiper, SwiperSlide } from "swiper/react";
import { Autoplay } from "swiper/modules";
import GalleryCategoryTabs from "./galleryCategoryTabs";
...
const Gallery = () => {
  ...
  const getImages = () => {
    if (key === "starLigth") {
      return galleryImages.filter(image => image.key === "starLigth" && image.category === "축제사진");
    } else {
      return galleryImages.filter(image => image.key === "moonLight" && image.category === "축제사진");
    }
  };
